  ...
  if (isIndex && projects.length > 0) {
    return (
      <ProjectsDrawer projects={projects}>
        <button className={`${isActive ? 'bg-primary text-primary-foreground' : 'bg-card text-primary'} font-mono uppercase rounded-sm text-xxs pl-2 pr-2.5 min-h-7 pt-2 pb-2 leading-none hover:bg-primary hover:text-primary-foreground transition-colors grow md:grow-0 text-left`}>
          [I]{` `}{title}
        </button>
      </ProjectsDrawer>
    )
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-xs md:max-w-md aspect-[5/6] border-none p-4 rounded-md">
          ...
          <div className="grid gap-2 grow md:grow-0">
            <Link href="mailto:email@example.com" className="group flex items-center gap-1 text-sm hover:text-primary-foreground hover:bg-primary transition-all bg-card rounded-sm px-4 py-2 justify-between">
              Email
              <SquareArrowOutUpRight size={14} className='text-primary-foreground group-hover:opacity-100 opacity-0 transition-all' />
            </Link>
            <Link href="https://github.com/" target="_blank" className="group flex items-center gap-1 text-sm hover:text-primary-foreground hover:bg-primary transition-all bg-card rounded-sm px-4 py-2 justify-between">
              GitHub
              <SquareArrowOutUpRight size={14} className='text-primary-foreground group-hover:opacity-100 opacity-0 transition-all' />
            </Link>
            <Link href="https://linkedin.com/" target="_blank" className="group flex items-center gap-1 text-sm hover:text-primary-foreground hover:bg-primary transition-all bg-card rounded-sm px-4 py-2 justify-between">
              LinkedIn
              <SquareArrowOutUpRight size={14} className='text-primary-foreground group-hover:opacity-100 opacity-0 transition-all' />
            </Link>
            <Link href="https://twitter.com/" target="_blank" className="group flex items-center gap-1 text-sm hover:text-primary-foreground hover:bg-primary transition-all bg-card rounded-sm px-4 py-2 justify-between">
              Instagram
              <SquareArrowOutUpRight size={14} className='text-primary-foreground group-hover:opacity-100 opacity-0 transition-all' />
            </Link>
          </div>
